...
本目录是**动态插件（零安装）路径**的源码：动态 Cordis 插件 `apibal-1` 的两个代码半部分（与 `cordis_define` 工具参数 `code.host` / `code.client` 一一对应）。
...
仓库根目录另有**正式插件包**版本（`lib/index.js` + `client/client.js` + `cordis.patch.yml` + `package.json`），可安装进 DSH profile（推荐）。
