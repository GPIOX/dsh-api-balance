/**
 * dsh-api-balance host entry: mounts the /dsh-api-balance/* HTTP routes once
...
export function apply(ctx, config) {
  ctx.inject(['webServer'], (host) => {
    host.effect(() => {
      ...
      // GET /dsh-api-balance/status — which providers have a saved key.
      disposers.push(host.webServer.register({
        kind: 'exact',
        path: '/dsh-api-balance/status',
      ...
      // POST /dsh-api-balance/save — store a key in the credential store.
      disposers.push(host.webServer.register({
        kind: 'exact',
        path: '/dsh-api-balance/save',
        handler: async (request, response) => {
      ...
      // POST /dsh-api-balance/clear — remove a stored key.
      disposers.push(host.webServer.register({
        kind: 'exact',
        path: '/dsh-api-balance/clear',
        handler: async (request, response) => {
          if (request.method !== 'POST') {
            response.writeHead(405, { allow: 'POST' })
            response.end()
      ...
      // GET /dsh-api-balance/fetch — query one provider's balance.
      disposers.push(host.webServer.register({
        kind: 'exact',
        path: '/dsh-api-balance/fetch',
        handler: async (request, response) => {
          if (request.method !== 'GET') {
            response.writeHead(405, { allow: 'GET' })
            response.end()
