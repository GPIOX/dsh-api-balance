 ...
 * the profile composes the webServer service. Balance lookups use native Node.js fetch
 * (no shell/curl dependency). API keys live in the Harness credential store
...
async function fetchBalance(host, args) {
  ...
  // Use native fetch instead of shell/curl to avoid PowerShell alias issues on Windows
  // (where `curl` maps to Invoke-WebRequest and breaks all curl-specific flags).
  try {
